...
func (p *Plugin) CobraAddCommands(rootCmd *cobra.Command) error {
	...
	var metaCmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cli.Println(fmt.Sprint(verbose))

			return meta(args[0], args[1], keyringPassphrase, username, password, override, clean, verbose, p.k)
		},
	}
...
func meta(environment, tags, keyringPassphrase, username, password, override string, clean bool, verbose int, k keyring.Keyring) error {
	...
	verbosity := ""
	if verbose > 0 {
		verbosity = "-" + strings.Repeat("v", verbose)
	...
	go func(wg *sync.WaitGroup) {
		defer wg.Done()
		deployCmd := exec.Command("plasmactl", "platform:deploy", environment, tags, verbosity)
